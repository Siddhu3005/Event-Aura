              ...
              <div className="grid grid-auto" style={{ marginTop: '1rem' }}>
                {profileData.events.ongoing.map((event: any) => (
                  <div key={event._id} className="card">
                    ...
                    <p>Community: {event.communityId?.name}</p>
                ...
                <div key={index} style={{
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <span className={`badge ${
                      activity.type === 'community_joined' ? 'badge-active' :
                      activity.type === 'event_registered' ? 'badge-upcoming' :
                      activity.type === 'certificate_earned' ? 'badge-completed' :
                      'badge-active'
                    }`}>
                  ...
                  <p style={{ color: '#6b7280', fontSize: '0.875rem', margin: '0.5rem 0 0 0' }}>
                    {new Date(activity.date).toLocaleDateString()} • {activity.community || 'EventAura'}
                  </p>
                </div>
              )) : (
                <p style={{ textAlign: 'center', color: '#6b7280' }}>No activities yet. Join communities and register for events to see your activity timeline!</p>
              )}
            </div>
          </div>
        );
            ...
            <div className="grid grid-auto" style={{ marginTop: '1rem' }}>
              {profileData.user.communitiesJoined.map((community: any) => (
                <div key={community._id} className="card">
                  ...
                  <p>Events: {community.events?.length || 0}</p>
                  <div style={{ marginTop: '1rem' }}>
                    <button 
